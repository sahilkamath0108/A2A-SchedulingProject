# 🗓️ Pretend this is Grace's Calendar
FAKE_AVAILABILITY = {
...
def get_availability(date_str: str) -> dict[str, str]:
    """
    Simulates checking Grace's availability on a specific date.
    ...
    if availability:
        return {
            "status": "completed",
            "message": f"On {date_str}, Grace is {availability}.",
        }

    return {
        "status": "input_required",
        "message": f"Grace is not available on {date_str}. Please ask about another date.",
    }